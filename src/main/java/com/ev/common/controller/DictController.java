...
public class DictController extends BaseController {
	...
	public PageUtils listForMap(@RequestParam Map<String, Object> params) {
		// 查询列表数据
		Query query = new Query(params);
		List<Map<String,Object>> dictList = dictService.listForMap(query);
		int total = dictService.countForMap(query);
		PageUtils pageUtils = new PageUtils(dictList, total);
		return pageUtils;
	}
	...
	public R update(@ModelAttribute DictDO dict) {
		if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
			return R.error(1, "演示系统不允许修改,完整体验请部署程序");
		}
	...
	@ResponseBody
	public List<DictDO> getDictByType(@PathVariable("type") String type) {
