...
//		results.put("userList",userList);
		...
		//获取附件
		Map<String,Object> contentMap = new HashMap<String,Object>(){{put("assocId",monthReport.getId());put("assocType",Constant.MONTH_REPORT_APPEARANCE_ATTACHMENT);}};
		List<ContentAssocDO> contentAssocDOS = contentAssocService.list(contentMap);
		results.put("initFileList", contentAssocDOS);
		//获取发送对象
		List<Map<String,Object>> targetList = userAssocService.list(new HashMap<String,Object>(){{put("assocId",monthReport.getId());put("assocType",Constant.MONTH_REPORT_TARGET);}});
		results.put("targetList", targetList);
		//获取回复信息
		Map<String,Object> commentMap = new HashMap<String,Object>(){{put("assocId",id);put("assocType",Constant.MONTH_REPORT_COMMENT);}};
//		List<CommentDO> commentList = commentService.list(commentMap);
	...
	public void commentMonthReport(Long monthReportId, String comment) {
		CommentDO commentDo = new CommentDO(monthReportId,Constant.MONTH_REPORT_COMMENT,comment);
	...
	public void saveMonthReport(MonthReportDO monthReportDO, Long[] targetList, String[] tagLocationAppearanceAttachment, String[] deleteTagAppearanceAttachment){
		if(monthReportDO.getId() == null || monthReportDO.getId() == 0){
			...
			for(int i=0;i<targetList.length;i++){
				UserAssocDO userAssocDO = new UserAssocDO(monthReportDO.getId(),Constant.MONTH_REPORT_TARGET,targetList[i]);
		...
		//附件信息操作
		contentAssocService.saveList(monthReportDO.getId(),tagLocationAppearanceAttachment,Constant.MONTH_REPORT_APPEARANCE_ATTACHMENT);
	...
	public void allPowerfulMelthod(MonthReportDO monthReportDO, Long[] targetList, String[] taglocationappearanceImage, int sign){
		...
		if(sign==1){monthReportDO.setStatus(Constant.APPLY_APPROED);} //148 已提交
		...
		query.put("assocType", Constant.MONTH_REPORT_TARGET);
		userAssocService.removeByAssocIdAndUserId(query);

		//保存新增发送人
		if(targetList.length>0){
			for(int i=0;i<targetList.length;i++){
				UserAssocDO userAssocDO = new UserAssocDO(id,Constant.MONTH_REPORT_TARGET,targetList[i]);
				userAssocService.save(userAssocDO);
			}
		}
		//删除路径
		...
		Long[] ids = new Long[1];
		ids[0] =id;
		contentAssocService.removeByAssocIdAndType(ids, Constant.MONTH_REPORT_APPEARANCE_ATTACHMENT);

		if(taglocationappearanceImage.length>0){
			contentAssocService.saveList(id,taglocationappearanceImage,Constant.MONTH_REPORT_APPEARANCE_ATTACHMENT);
		}
	}
	...
	public R listOfCanDelet(Map<String, Object> map,Long[]ids) {
		...
		if(Objects.equals(mapList.size(),ids.length)){
			...
			contentAssocService.removeByAssocIdAndType(deleIds, Constant.MONTH_REPORT_APPEARANCE_ATTACHMENT);
